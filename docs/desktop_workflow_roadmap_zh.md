...
- 桌面打包：MVP 稳定后使用 Tauri 或 Electron；若希望更轻量，优先 Tauri
...
**目标**：将稳定的本地 Web 应用打包为私人桌面应用。

**推荐方案**：

- 保持 React 前端 + FastAPI 后端架构
- MVP 可用后再打包前端为桌面壳
- 若希望更轻量且可接受 Rust 工具链，优先 Tauri
- 若 Python 后端 sidecar 打包更方便，可选 Electron
- 继续使用 SQLite 作为本地私有存储

**具体工作**：

- 构建前端生产资源
- 将后端打包为本地可执行文件或 sidecar 进程
- 桌面应用启动时自动启动本地后端
- 前端 API 指向本地后端
- 数据库存放在应用数据目录
- 添加本地设置：数据路径、刷新行为、可选 API Token
- 构建 Windows 桌面版供个人使用
...
| 周次 | 内容 |
|------|------|
| 第 1 周 | 项目骨架、FastAPI、React、SQLite、单股日 K、第一张图表 |
| 第 2 周 | 股票搜索、K 线 API、本地缓存、个股详情页完善 |
| 第 3 周 | 自选股与 Dashboard |
| 第 4 周 | 价格位置分数与顶底区域展示 |
| 第 5 周 | 个股资金流存储与图表 |
| 第 6 周 | 板块数据与板块资金流 |
| 第 7 周 | 排行榜与每日复盘页 |
| 第 8 周 | 实时看盘第一版：实时行情、分钟 K、分时图、自选股快刷 |
| 第 9 周 | 稳定性、测试、数据质量、桌面打包概念验证 |
...
| ID | 内容 | 状态 |
|----|------|------|
| foundation | 搭建仓库、后端、前端、SQLite 与健康检查通信 | 进行中 / 部分完成 |
| kline-mvp | 拉取、存储、提供并渲染个股日 K 与成交量 | 部分完成 |
| watchlist-dashboard | 股票搜索、自选股管理、每日 Dashboard | 待开始 |
| position-indicator | 价格位置分数与顶底区域可视化 | 待开始 |
| moneyflow | 个股与板块资金流采集、存储、API 与图表 | 待开始 |
| sector-ranking | 板块页、排行榜、每日复盘流程 | 待开始 |
| realtime-market-watch | 秒级报价、分钟 K、分时图、自选股盘中快刷 | 待开始 |
| desktop-packaging | 打包为私人 Windows 桌面应用 | 待开始 |
